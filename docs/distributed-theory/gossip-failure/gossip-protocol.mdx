...
**Gossip 消息**是传播的载体，可以是任意需要同步的数据：节点状态、数据副本、拓扑信息。消息体通常包含版本号或时间戳，用于判断数据的「新旧」程度。

**周期性的 gossip 交互**是传播的驱动力。每个节点按固定间隔（通常几百毫秒到几秒）主动发起一次 gossip 轮次。这个间隔是延迟与带宽的权衡——间隔越短，收敛越快，但网络负载越高。

**随机选择传播目标**是 Gossip 的精髓。每一轮中，节点从集群中随机选择若干个节点（通常是 1-3 个）作为消息的传播目标。这种随机性保证了：
