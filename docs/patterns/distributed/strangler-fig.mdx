...
双写很难保证绝对一致，因为存在「写成功但网络超时」等边界情况。解决方案包括：1）异步双写 + 最终一致，允许短暂不一致；2）事务性双写，使用分布式事务（如 Seata）保证原子性；3）Change Data Capture（CDC），通过监听数据库变更日志同步数据；4）迁移完成后进行数据校验，发现不一致及时恢复。实际项目通常组合使用这些方案。
